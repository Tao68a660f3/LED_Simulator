...
class BmpCreater():
    ...
    def __init__(self,Manager=FontManager(),color_type="RGB",color=(255,255,255),ch_font="",asc_font="",only_sysfont = False,relative_path = "./resources/"):
    ...
    def hconcat_images(self,image_list = [], vertical = False, space = 1, style = 0):
        # style: -1,0,1，对齐方式
        if len(image_list) == 0:
            return Image.new(self.color_type,(1,1))
        if not vertical:
            # 计算所有图像的最大高度和宽度之和
            ...
            if task in self.FontManager.icon_dict.keys():
                ...
                for chr in font_tasks:
                    if chr.isascii() and not self.only_sysfont:
                        ch = self.ASC_Reader.get_text_bmp(chr,y_offset)
                    else:
...
if __name__ == "__main__":
    ch_font="HZK16宋体"
    asc_font="ASC1610"
    FontCreater = BmpCreater(Manager=FontManager(),color_type="RGB",color=(255,200,0),ch_font=ch_font,asc_font=asc_font,only_sysfont = False,relative_path = "./resources/")
    font_img = FontCreater.create_character(vertical=1, text="`wheelchair32x32_2`欢迎乘坐200路外环公交车！(田螺山)", ch_font_size=24, ch_bold_size_x=2, ch_bold_size_y=1, space=0, scale=80, auto_scale=0, scale_sys_font_only=1, new_width = 80, new_height = 32, y_offset = 0, style = 0)
