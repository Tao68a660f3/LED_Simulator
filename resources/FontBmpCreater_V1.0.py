...
class ASC_font_Reader():
    def __init__(self,font_path = font_relative_path+"ASC2412.font"):
        self.font_path = font_path
        self.font_path = font_relative_path + "fontFont/" + self.font_path
        self.font_hex_list = []
    ...
    def get_text_bmp(self,asc,y_offset = 0):
        ...
        image = image.crop((-1,y_offset,data[0],y_offset+self.bitmap_size[1]))
        return image
    
class ASC_Bmp_Reader():
    def __init__(self,fontBmpPath):
        self.fontBmpPath = fontBmpPath
        self.fontBmpPath = font_relative_path + "bmpFont/" + self.fontBmpPath
        self.fnt_img = Image.open(self.fontBmpPath)
        self.ascii_size = [int(fontBmpPath.split(".")[0].split("_")[1].split("-")[0]),int(fontBmpPath.split(".")[0].split("_")[1].split("-")[1])]
    ...
    def get_text_bmp(self,asc,y_offset=0):
        ...
        if self.ascii_size[0] == self.ascii_size[1]:
            ...
            r = r+1 if r+1 <= self.ascii_size[0] else r
        else:
            l = 0
            r = self.ascii_size[0]
...
class HZK_Font_Reader():
    def __init__(self,fontPath):
        ...
        self.fontPath = font_relative_path + "binFont/" + self.fontPath
...
class Sys_Font_Reader():
    ...
    def get_text_bmp(self,text,y_offset=0,font_size=16,xb=1,yb=1,scale=100):
        ...
        image = Image.new("1", (text_width, text_height))
        # 获取新的Draw对象
        draw = ImageDraw.Draw(image)
        # 设置字体，绘制文本，加粗
        for i in range(xb):
            for j in range(yb):
                draw.text((i, j-y_offset), text, font=self.font, fill=1, anchor="la")
...
class BmpCreater():
    ...
    def __init__(self,ch_font="",asc_font="",only_sysfont = False):
        self.ch_font = ch_font
        self.asc_font = asc_font
        self.only_sysfont = only_sysfont

        asc_font_type = asc_font.split(".")[-1].lower()
        ch_font_type = ch_font.split(".")[-1].lower()
        if not only_sysfont:
            if asc_font_type == "font":
                self.ASC_Reader = ASC_font_Reader(asc_font)
            elif asc_font_type == "bmp":
                self.ASC_Reader = ASC_Bmp_Reader(asc_font)
        if not self.only_sysfont:
            if ch_font_type == "bin":
                self.Ch_Reader = HZK_Font_Reader(ch_font)
            else:
                self.Ch_Reader = Sys_Font_Reader(ch_font)
        else:
            if ch_font_type == "bin":
    ...
    def hconcat_images(self,image_list = [], vertical = False, space = 1, style = 0):
        ...
        if not vertical:
            ...
            # 创建一个新的图像作为画布，背景为白色
            new_image = Image.new('1', (total_width+space*(len(image_list)-1), total_height))
        else:
            total_height = sum(image.height for image in image_list)
            total_width = max(image.width for image in image_list)
            new_image = Image.new('1', (total_width, total_height+space*(len(image_list)-1)))        
        # 在新图像上依次粘贴每个图像
        x_offset = 0
        y_offset = 0
        if style > 0:
    ...
    def create_character(self,vertical=False, text="", ch_font_size=16, ch_bold_size_x=2, ch_bold_size_y=1, space=0, scale=100, auto_scale=False, scale_sys_font_only=False, new_width = None, new_height = None, y_offset = 0, style = 0):
        ...
        tasks = list(text)
        for chr in tasks:
            if chr.isascii() and not self.only_sysfont:
                IMAGES.append(self.ASC_Reader.get_text_bmp(chr,y_offset))
            else:
                if scale_sys_font_only:
                    sscale = scale
                else:
                    sscale = 100
                IMAGES.append(self.Ch_Reader.get_text_bmp(chr,y_offset,ch_font_size,ch_bold_size_x,ch_bold_size_y,sscale))     
        # 拼接图像
...
if __name__ == "__main__":
    ch_font = asc_font = ""
    ch_font = "arial.ttf"
    asc_font = "ASCII_8-16.bmp"
    FontCreater = BmpCreater(ch_font,asc_font,1)
    font_img = FontCreater.create_character(vertical=0, text="欢迎乘坐123456789200路外环无人售票车！无障碍", ch_font_size=36, ch_bold_size_x=1, ch_bold_size_y=1, space=0, scale=100, auto_scale=0, scale_sys_font_only=0, new_width = 100, new_height = 16, y_offset = 0, style = 0)
    font_img.save("混合字体测试生成.bmp")
    # ch_font = "hzk16s.bin"
    # asc_font = "ASC1608.font"
    # FontCreater = BmpCreater(ch_font,asc_font,False)
    # font_img = FontCreater.create_character(vertical=0, text="欢迎乘坐２００路外环无人售票车！200外开往永定门汽车站", ch_font_size=16, ch_bold_size_x=2, ch_bold_size_y=1, space=0, scale=100, auto_scale=0, scale_sys_font_only=0, new_width = 100, new_height = 16, y_offset = 0, style = 0)
    # font_img.save("混合字体测试生成2.bmp")