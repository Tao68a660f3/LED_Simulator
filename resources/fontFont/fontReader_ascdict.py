...
font_path = "./resources/fontFont/ASC2412.font"
...
def create_ascii_dictmap():
    imglist = []
    for a in range(128):
        c = chr(a)
        imglist.append(FileReader.findSingleAscii(c))

    h = max(image.height for image in imglist)
    w = max(image.width for image in imglist)

    if w < h:
        w = h

    imgbmp = Image.new("1",(w*16,h*16))
    xpos,ypos = 0,0

    for x in range(128):
        imgbmp.paste(imglist[x], (xpos, ypos))
        xpos += w
        if (x+1) % 16 == 0:
            ypos += h
            xpos = 0

    imgbmp = imgbmp.point(lambda x: not x)

    return imgbmp

if __name__ == "__main__":
    FileReader = ASC_file_reader(font_path)
    bmp = create_ascii_dictmap()
    bmp.save("测试生成_img.bmp")
